...
BMI::BMI(double height, double weight)
{
    ...
    if (height <= 0 || weight <= 0){
        // throw an exception
        throw invalid_argument("Height or Weight is less then or equal to 0");
    }
...
void BMI::setHeight(double height)
{
    ...
    if (height <= 0 ){
        // throw an exception
        throw invalid_argument("Height is less then or equal to 0");
    }
    this->height = height;
    calculate();
}
...
void BMI::setWeight(double weight)
{
    ...
    if (weight <= 0 ){
        // throw an exception
        throw invalid_argument("Weight is less then or equal to 0");
    }
    this->weight = weight;
    calculate();
}
...
{
    // place scale field value in specified scale
    height = this->height;
    weight = this->weight;
    bmi = this->bmi;
}
